...
export const EXPECTED_UNRESOLVED = Object.freeze({
  ...
  'dining:smith-dining': 'no operating period in the Columbia Dining feed for Fall 2026; revisit if it reopens',
});

const RECREATION_UNAVAILABLE = new Set(['Hours need verification', 'Separate hours not published']);
/**
 * A status can be an answer ("Closed") or an admission that there is none. The dining
 * schema uses a status string for both, so the second kind has to be named — otherwise a
 * venue reading "Hours not published" for fourteen days counts as fourteen answered days
 * and this check stays silent, which is exactly what it exists to prevent.
 */
const DINING_UNAVAILABLE = new Set(['Hours not published', 'Hours need verification']);

const hasIntervals = day => Array.isArray(day?.intervals) && day.intervals.length > 0;
const answeredDay = (day, unavailable) => hasIntervals(day) || Boolean(day?.status && !unavailable.has(day.status));
...
export const VENUE_READERS = Object.freeze({
  ...
  dining: snapshot => (snapshot?.locations || []).map(location => ({
    id: location.id,
    usableDays: (location.days || []).filter(day => answeredDay(day, DINING_UNAVAILABLE)).length,
    totalDays: (location.days || []).length,
  })),
  'barnard-dining': snapshot => (snapshot?.venues || []).map(venue => ({
    id: venue.id,
    usableDays: (venue.days || []).filter(day => answeredDay(day, DINING_UNAVAILABLE)).length,
  ...
  recreation: snapshot => (snapshot?.facilities || []).flatMap(facility => [
    ...
    id: entry.id,
    usableDays: (entry.days || []).filter(day => answeredDay(day, RECREATION_UNAVAILABLE)).length,
