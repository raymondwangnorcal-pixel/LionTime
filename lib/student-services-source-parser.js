...
export function parseLernerSource({ homeHtml, calendarHtml, calendarText, calendarUrl }) {
  ...
  if (!/Building Hours/i.test(content) || !/Lerner Hall/i.test(content)) {
    throw new Error('Lerner building-hours content is missing');
  }
  const hasCalendar = Boolean(calendarUrl);
  let sameOriginEvents = false;
  let embeddedGoogleCalendar = false;
  if (hasCalendar) {
    const officialCalendar = new URL(calendarUrl, STUDENT_SERVICES_SOURCE_URLS.lerner);
    sameOriginEvents = officialCalendar.origin === new URL(STUDENT_SERVICES_SOURCE_URLS.lerner).origin
      && officialCalendar.pathname === '/events';
    embeddedGoogleCalendar = officialCalendar.hostname === 'calendar.google.com'
  ...
  const baseline = [];
  for (const match of content.matchAll(/((?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)(?:\s*[-&]\s*(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday))?)[\s:]+(\d{1,2}(?::\d{2})?\s*[AP]M\s*[-–—]\s*\d{1,2}(?::\d{2})?\s*[AP]M)/gi)) {
    const rangeText = match[2];
    const rangeMatch = text(rangeText).match(/(\d{1,2}(?::\d{2})?\s*(?:[AP]\.?\s*M\.?)?)\s*(?:[-–—]|\bto\b)\s*(\d{1,2}(?::\d{2})?\s*[AP]\.?\s*M\.?)/i);
    if (!rangeMatch) throw new Error(`invalid Lerner baseline range: ${rangeText}`);
    const closingMeridiem = rangeMatch[2].match(/([AP])\.?\s*M\.?/i)?.[0];
    const openingText = /[AP]\.?\s*M\.?/i.test(rangeMatch[1]) ? rangeMatch[1] : `${rangeMatch[1]} ${closingMeridiem}`;
    const open = parseClock(openingText);
    const close = parseClock(rangeMatch[2], true);
    const crossesMidnight = close !== '24:00' && close <= open;
    baseline.push(evidence({ sourceId: 'lerner', targetId: 'lerner', type: 'open-access',
      weekdays: dayIndexes(match[1]), intervals: [[open, crossesMidnight ? '24:00' : close]], suffix: `baseline-${baseline.length}` }));
  }
...
function parseRawMailSource($) {
  ...
  const hasSummerHours = /Summer Hours \(through 8\/30\)/i.test(content);
  // Past periods drop off the page; only the current and upcoming ones are required.
  // Fall Rush is one of them — it ends 2026-09-27 and will be removed like the rest, so
  // requiring it would break this source on that date for no reason.
  const hasCheckInWeek = /Check-In Week: August 31\s*[-–—]\s*September 7/i.test(content);
  const hasFallRush = /Fall Rush: September 8\s*[-–—]\s*27/i.test(content);
  ...
  const checkIn = hasCheckInWeek ? sectionBetween(/Check-In Week/i, /Fall Rush/i) : null;
  const rush = hasFallRush ? sectionBetween(/Fall Rush/i, /Regular Fall Hours/i) : null;
  ...
  if (hasSummerHours) {
    ...
    records.push(rawMailRecord(summer, { label: 'Summer Hours', weekdays: [1, 2, 3, 4, 5], pattern: new RegExp(`Monday\\s*[-–—]\\s*Friday\\s*:\\s*${range}`, 'i'), start: '2026-01-01', end: '2026-08-30' }));
  }
  if (checkIn) {
    records.push(
  ...
  if (hasFallRush) {
    ...
    // The weekend has appeared both as a combined "Saturday & Sunday" line and, later,
    // as separate Saturday/Sunday lines (Sunday closed) — support both shapes.
    const combinedWeekend = new RegExp(`Saturday\\s*&\\s*Sunday\\s*:\\s*${range}`, 'i');
    if (combinedWeekend.test(rush)) {
      records.push(rawMailRecord(rush, { label: 'Fall Rush', weekdays: [0, 6], pattern: combinedWeekend, start: '2026-09-08', end: '2026-09-27' }));
    } else {
      records.push(
        rawMailRecord(rush, { label: 'Fall Rush', weekdays: [6], pattern: new RegExp(`Saturday\\s*:\\s*${range}`, 'i'), start: '2026-09-08', end: '2026-09-27' }),
        rawMailClosedRecord(rush, { label: 'Fall Rush', weekdays: [0], pattern: /Sunday\s*:\s*CLOSED/i, start: '2026-09-08', end: '2026-09-27' }),
      );
    }
  }
  records.push(
    ...
    // The page says "Saturday & Sunday: CLOSED" for this period. Without a record for it
    // the weekend has no evidence at all from 2026-09-28 and reads as unverified instead
    // of closed. Absence is asserted, not assumed: a reworded line fails loudly here.
    rawMailClosedRecord(regular, { label: 'Regular Fall Hours', weekdays: [0, 6],
      pattern: /Saturday\s*&\s*Sunday\s*:\s*CLOSED/i, start: '2026-09-28', end: '2026-12-31' }),
  );
...
function rawMailClosedRecord(content, { label, weekdays, pattern, start, end }) {
  if (!pattern.test(content)) throw new Error(`Student Life live closure is missing: ${label}`);
  return evidence({
    sourceId: 'mail', targetId: 'mail-center', type: 'office-hours', weekdays,
    intervals: [], status: 'Closed', effectiveStart: start, effectiveEnd: end,
    reason: label, suffix: `${start}-${weekdays.join('')}`,
  });
}

const WEEKDAY_ALT = '(?:Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday)';
const WEEKDAY_LIST = new RegExp(`^${WEEKDAY_ALT}(?:\\s*(?:,|&|-|–|—|and)\\s*${WEEKDAY_ALT})*$`, 'i');
// A comma may be followed by "and" (Oxford comma): "Monday, Tuesday, Thursday, and Friday".
const WEEKDAY_LINE = new RegExp(`^(${WEEKDAY_ALT}(?:\\s*(?:,\\s*(?:and\\s+)?|&|-|–|—|\\band\\b)\\s*${WEEKDAY_ALT})*)\\s*:?\\s*(.+)$`, 'i');
const ALL_WEEK = [0, 1, 2, 3, 4, 5, 6];

function classifyDrupalLabels(labels, fallback) {
  const dropIn = labels.find(label => /drop-in/i.test(label.text));
  if (dropIn) return /virtual/i.test(dropIn.text) ? 'virtual-only' : 'walk-in';
  if (labels.some(label => label.strong && /^virtual\b/i.test(label.text.trim()))) return 'virtual-only';
  if (labels.some(label => /appointment/i.test(label.text))) return 'appointment-only';
  if (labels.some(label => /after\s*hours|24\/7/i.test(label.text))) return 'phone-support';
  return fallback;
}

function drupalLabel(node) {
  const strong = node.find('strong').first();
  return strong.length ? { text: text(strong.text()), strong: true } : { text: text(node.text()), strong: false };
}

function drupalDlPairs($, dl) {
  const dts = $(dl).children('dt').toArray();
  const dds = $(dl).children('dd').toArray();
  return dts.map((dt, index) => ({ dt: $(dt), dd: $(dds[index]) })).filter(pair => pair.dd.length);
}

function drupalDdLines(dd) {
  const items = dd.find('li');
  if (items.length) {
    const lines = [];
    for (let index = 0; index < items.length; index += 1) lines.push({ text: text(items.eq(index).text()), header: false });
    return lines.filter(line => line.text);
  }
  const html = dd.html() || '';
  return html.split(/<br\s*\/?\s*>/i).map(fragment => {
    const strongMatch = fragment.match(/^\s*<strong>([\s\S]+)<\/strong>\s*$/i);
    const value = text(load(`<div>${fragment}</div>`)('div').text());
    const header = Boolean(strongMatch) && !/\d/.test(value) && !WEEKDAY_LINE.test(value);
    return { text: value, header };
  }).filter(line => line.text);
}

function drupalWeekdayRange(line) {
  const match = line.match(WEEKDAY_LINE);
  if (!match) return null;
  let weekdays;
  try { weekdays = dayIndexes(match[1]); } catch { return null; }
  if (/^closed\b/i.test(match[2])) return { weekdays, closed: true };
  try { return { weekdays, interval: parseRange(match[2]) }; } catch { return null; }
}

function drupalRecordsFromDl($, dl, targetId, defaultType, headingLabel = null) {
  const pairs = drupalDlPairs($, dl);
  const hoursIndex = pairs.findIndex(pair => /^hours$/i.test(text(pair.dt.text())));
  if (hoursIndex < 0) return [];
  // Several drop-in programmes share a weekday with different hours (CAPS lists five), and
  // the resolver treats same-type records with different intervals as ambiguous unless
  // they carry distinct reasons — so label each list by its programme, else by its heading.
  const labels = pairs.filter((_pair, index) => index !== hoursIndex)
    .map(pair => ({ ...drupalLabel(pair.dd), text: drupalLabel(pair.dd).text.replace(/:$/, '').trim() }))
    .filter(label => label.text && !WEEKDAY_LIST.test(label.text));
  const afterHoursLabel = labels.find(label => /after\s*hours/i.test(label.text));
  const programme = labels.find(label => label.strong && label !== afterHoursLabel);
  const reason = programme?.text || afterHoursLabel?.text || headingLabel || null;
  const contextLabels = pairs.filter((_pair, index) => index !== hoursIndex)
    .flatMap(pair => [drupalLabel(pair.dt), drupalLabel(pair.dd)]);
  let subType = classifyDrupalLabels(contextLabels, defaultType);
  const lines = drupalDdLines(pairs[hoursIndex].dd);
  const results = [];
  let sawWeekdayLine = false;
  for (const line of lines) {
    if (/available 24\/7/i.test(line.text)) {
      results.push({ weekdays: ALL_WEEK, interval: ['00:00', '24:00'], type: 'phone-support' });
      sawWeekdayLine = true;
      continue;
    }
    if (line.header) { subType = classifyDrupalLabels([{ text: line.text, strong: true }], subType); continue; }
    const parsed = drupalWeekdayRange(line.text);
    if (!parsed) continue;
    sawWeekdayLine = true;
    results.push(parsed.closed
      ? { weekdays: parsed.weekdays, closed: true, type: subType }
      : { weekdays: parsed.weekdays, interval: parsed.interval, type: subType });
  }
  if (!sawWeekdayLine && hoursIndex > 0) {
    const label = drupalLabel(pairs[hoursIndex - 1].dd).text.replace(/:$/, '').trim();
    const afterHours = /after\s*hours/i.test(label);
    const weekdays = afterHours ? ALL_WEEK : (WEEKDAY_LIST.test(label) ? dayIndexes(label) : null);
    if (weekdays) {
      const type = afterHours ? 'phone-support' : subType;
      for (const line of lines) {
        if (/available 24\/7/i.test(line.text)) { results.push({ weekdays: ALL_WEEK, interval: ['00:00', '24:00'], type: 'phone-support' }); continue; }
        if (/^closed$/i.test(line.text)) { results.push({ weekdays, closed: true, type }); continue; }
        try { results.push({ weekdays, interval: parseRange(line.text), type }); } catch { /* not a range line */ }
      }
    }
  }
  return results.map((record, index) => evidence({ sourceId: 'health', targetId, type: record.type,
    weekdays: record.weekdays, intervals: record.closed ? [] : [record.interval],
    status: record.closed ? 'Closed' : null, reason,
    suffix: `${targetId}-${record.type}-${record.weekdays.join('')}-${index}` }));
}

function parseDrupalTableHealthSource($) {
  // The live page carries visually-hidden chrome headings ("Main navigation expanded",
  // "You are here:") ahead of the content, so find the schedule heading rather than
  // trusting document order.
  const heading = $('h2').filter((_index, el) => /Operating Hours/i.test(text($(el).text())));
  if (!heading.length) throw new Error('Health live schedule signature is missing');
  const sections = [
    ['alice-health', /^Alice! Health Promotion$/i, 'office-hours'],
    ['caps', /^Counseling and Psychological Services$/i, 'office-hours'],
    ['disability', /^Disability Services$/i, 'appointment-only'],
    ['medical', /^Medical Services$/i, 'office-hours'],
    ['svr', /^Sexual Violence Response$/i, 'office-hours'],
    ['student-insurance', /^Student Insurance Office$/i, 'appointment-only'],
    ['immunization', /^Immunization Compliance Office$/i, 'virtual-only'],
  ];
  const records = [];
  for (const [targetId, headingPattern, defaultType] of sections) {
    const heading3 = $('h3').filter((_index, el) => headingPattern.test(text($(el).text()))).first();
    if (!heading3.length) throw new Error(`Health service section is missing: ${targetId}`);
    const section = heading3.closest('.paragraph--type--cu-page-slice');
    const nodes = section.find('h4, .table-def-list dl').toArray();
    if (!nodes.some(node => node.tagName === 'dl')) throw new Error(`Health service has no parseable availability: ${targetId}`);
    const before = records.length;
    let headingLabel = null;
    for (const node of nodes) {
      if (node.tagName === 'h4') { headingLabel = text($(node).text()) || null; continue; }
      records.push(...drupalRecordsFromDl($, node, targetId, defaultType, headingLabel));
    }
    if (records.length === before) throw new Error(`Health service has no parseable availability: ${targetId}`);
  }
  return records;
}
...
export function parseHealthSource(html) {
  const $ = load(html);
  if ($('.table-def-list').length) return parseDrupalTableHealthSource($);
...
function parseRawHealthSource($) {
  const content = text($('#main-article').text() || $('main').text());
  if (!/(?:Summer|Fall|Winter|Spring) 20\d{2} Operating Hours/i.test(content)) throw new Error('Health live schedule signature is missing');
  ...
  const capsFall = /Starting August 17, 2026/i.test(caps)
    ...
    : caps;
  // Alice! publishes either one Monday–Friday range (summer) or a Monday–Thursday
  // range plus a separate Friday range (fall). Take whichever shape is present.
  const aliceOffice = rangePattern('Monday\\s*[-–—]\\s*Friday').test(alice)
    ? [rawHealthAvailability(alice, { targetId: 'alice-health', type: 'office-hours', weekdays: [1, 2, 3, 4, 5], pattern: rangePattern('Monday\\s*[-–—]\\s*Friday'), reason: 'Office Hours', suffix: 'office-hours' })]
    : [
      rawHealthAvailability(alice, { targetId: 'alice-health', type: 'office-hours', weekdays: [1, 2, 3, 4], pattern: rangePattern('Monday\\s*[-–—]\\s*Thursday'), reason: 'Office Hours', suffix: 'office-weekdays' }),
      rawHealthAvailability(alice, { targetId: 'alice-health', type: 'office-hours', weekdays: [5], pattern: rangePattern('Friday'), reason: 'Office Hours', suffix: 'office-friday' }),
    ];
  const records = [
    ...aliceOffice,
    rawHealthAvailability(alice, { targetId: 'alice-health', type: 'virtual-only', weekdays: [1, 2, 4, 5], pattern: rangePattern('Monday, Tuesday, Thursday, Friday'), reason: 'Sexual & Reproductive Health Drop-In', suffix: 'virtual-drop-in' }),
