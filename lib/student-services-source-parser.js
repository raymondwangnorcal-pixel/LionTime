...
function drupalRecordsFromDl($, dl, targetId, defaultType) {
  ...
  return results.map((record, index) => evidence({ sourceId: 'health', targetId, type: record.type,
    ...
    status: record.closed ? 'Closed' : null, suffix: `${targetId}-${record.type}-${record.weekdays.join('')}-${index}` }));
}
...
function parseDrupalTableHealthSource($) {
  ...
  for (const [targetId, headingPattern, defaultType] of sections) {
    ...
    const dls = section.find('.table-def-list dl').toArray();
    if (!dls.length) throw new Error(`Health service has no parseable availability: ${targetId}`);
    const before = records.length;
    for (const dl of dls) records.push(...drupalRecordsFromDl($, dl, targetId, defaultType));
