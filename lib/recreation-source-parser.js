...
/**
 * `calendarNames` lists every name Columbia has published the calendar under. The name is
 * the last guard that the embed really is this venue's calendar, so an event under any
 * other name still rejects the whole payload — but a rename on Columbia's side must not.
 * On 2026-09-14 "University Hall (Blue) Gym" became "University Hall (Blue) Gym Open
 * Recreation" and Blue Gym sat on "Hours need verification" for a day and a half with
 * every scrape green; the hours audit is what caught it.
 */
const ACTIVITY_CALENDARS = Object.freeze({
  'blue-gym': Object.freeze({
    calendarNames: ['University Hall (Blue) Gym Open Recreation', 'University Hall (Blue) Gym'],
    title: 'Blue Gym',
    suppliesDodgeFallback: true,
  }),
  'levien-gymnasium': Object.freeze({ calendarNames: ['Levien Gymnasium Open Recreation'], title: null, suppliesDodgeFallback: false }),
  'aerobics-room-4': Object.freeze({ calendarNames: ['Aerobics Room 4 Open Recreation'], title: 'Aerobics Room 4 Open Recreation', suppliesDodgeFallback: false }),
  'functional-fitness-studio': Object.freeze({ calendarNames: ['Functional Fitness Studio Open Recreation'], title: 'Functional Fitness Studio Open Recreation', suppliesDodgeFallback: false }),
});
...
const WEEKDAY_NAME_GROUP = String.raw`(sunday|monday|tuesday|wednesday|thursday|friday|saturday)`;
const SEASONAL_HEADING_EXPRESSION = /\b(?:spring|summer|fall|winter)\s+(?:(?:semester|session)\s+)?(20\d{2})\b/i;
const SEMESTER_SCHEDULE_EXPRESSION = new RegExp(
  String.raw`Dodge Fitness Center\b[^.]{0,160}\b(?:academic\s+)?(?:semester|session)\s+schedule\s+from\s+`
  + WEEKDAY_NAME_GROUP + String.raw`,\s+([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?\s+(?:through|to|until)\s+`
  + WEEKDAY_NAME_GROUP + String.raw`,\s+([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?`,
  'i',
);
const BARNARD_LIVE_SCHEDULE_START = '2026-09-08';
...
export function parseColumbiaHours(html, { generated } = {}) {
  ...
  const effectiveRange = seasonalDateRange($, seasonalHeading, schedule)
    || seasonalSemesterDateRange($, seasonalHeading)
  ...
  if (seasonalLabel(seasonalHeading.text())) {
...
export function parseActivityCalendar(payload, { generated } = {}) {
  const definition = ACTIVITY_CALENDARS[payload?.targetId];
  if (!definition || !isOfficialActivityCalendar(payload?.calendarUrl, definition) || !validGeneratedDate(generated)) return [];
  ...
  for (const text of payload.weeks) {
    for (const event of calendarEvents(text)) {
      if (!definition.calendarNames.includes(event.calendarName)) return [];
  ...
  for (const [date, eventMap] of [...eventsByDate.entries()].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (blueIntervals.length) {
      ...
      if (definition.suppliesDodgeFallback) {
    ...
    const closures = events.filter(event => /\bclosed\b|\bclosure\b/i.test(event.title));
    ...
    evidence.push(createEvidence({
      ...
      effectiveRange: { start: date, end: date },
      dateIntervals: [],
      status: closures.some(event => /maintenance/i.test(event.title)) ? 'Closed for maintenance' : 'Closed',
      availabilityType: 'open-recreation',
    }));
  }
  return evidence;
...
export const parseBlueGymCalendar = parseActivityCalendar;
...
export function parseColumbiaModifications(html) {
  ...
  article.find('h2').each((_, dateHeading) => {
    ...
    const notices = $(dateHeading).nextUntil('h2').filter('h3, h4').add($(dateHeading).nextUntil('h2').find('h3, h4'));
    notices.each((_, noticeHeading) => {
      const targetId = targetIdFor($(noticeHeading).text());
      const paragraphs = $(noticeHeading).nextUntil('h2, h3, h4').filter('p').add($(noticeHeading).nextUntil('h2, h3, h4').find('p'))
  ...
  article.find('h2').filter((_, heading) => /recurring area closures/i.test($(heading).text())).each((_, heading) => {
    const notices = $(heading).nextUntil('h2').filter('h3, h4').add($(heading).nextUntil('h2').find('h3, h4'));
    notices.each((_, notice) => {
      const noticeText = boundedText($(notice).text());
      const targetId = targetIdFor(noticeText);
      if (!targetId) return;
      const details = $(notice).nextUntil('h2, h3, h4').text();
  ...
  const hasUnhandledKnownNotice = article.find('h3, h4').toArray().some(notice => (
...
export function isSafeEmptyColumbiaModificationsPage(html) {
  ...
  if (article.find('h3, h4').toArray().some(notice => targetIdFor($(notice).text()))) return false;
  ...
  for (const heading of recognizedHeadings) {
    const notices = $(heading).nextUntil('h2').filter('h3, h4').add($(heading).nextUntil('h2').find('h3, h4')).toArray();
...
export function parseBarnardHours(html, { generated } = {}) {
  const $ = cheerio.load(html);
  const section = $('section[aria-label*="Barnard Fitness Center"]').first();
  if (!section.length) return [];
  const heading = section.find('h1, h2, h3, h4').first().text();

  const weeklyIntervals = {};
  let hasUnparsedWeekdayRow = false;
  section.find('strong').each((_, element) => {
    const label = $(element).text();
    if (!new RegExp(`\\b(?:${WEEKDAYS.join('|')})\\b`, 'i').test(label)) return;
    const days = weekdayNumbers(label);
    const timeText = boundedText($(element).get(0).next?.data?.replace(/^[\s\u00a0]*:[\s\u00a0]*/, ''));
    const intervals = parseBarnardTimeRanges(timeText);
    if (!days || !intervals) {
      hasUnparsedWeekdayRow = true;
      return;
    }
    for (const day of days) weeklyIntervals[day] = [...(weeklyIntervals[day] || []), ...intervals];
  });
  const accessRestrictions = barnardAccessRestrictions(section.text());
  if (
    hasUnparsedWeekdayRow
    || !Array.from({ length: 7 }, (_, day) => weeklyIntervals[day]?.length > 0).every(Boolean)
    || !accessRestrictions.length
  ) return [];

  if (!seasonalLabel(heading)) return [];
  const generatedDate = validGeneratedDate(generated) ? easternDate(generated) : null;
  if (!generatedDate || generatedDate < BARNARD_LIVE_SCHEDULE_START) {
  ...
  const approvedStaleHeading = /\bsummer\s+2026\b/i.test(heading);
  if (!approvedStaleHeading && !barnardSeasonHeadingState(heading, generatedDate)) return [];

  const paragraphs = section.find('p').map((_, element) => $(element).text()).get();
  const effectiveCandidates = paragraphs.filter(value => /\beffective\b/i.test(value));
  if (effectiveCandidates.length > 1) return [];
  const effectiveRange = effectiveCandidates.length
    ? parseUnambiguousEffectiveRange(effectiveCandidates[0])
    : null;
  if (effectiveCandidates.length && !effectiveRange) return [];
  if (effectiveRange && (
    generatedDate < effectiveRange.start || generatedDate > effectiveRange.end
  )) {
    return [unavailableEvidence({
      targetId: 'barnard-fitness',
      sourceId: 'barnardFitness',
  ...
  const rollingEnd = addDays(generatedDate, 13);
  const liveEnd = effectiveRange && effectiveRange.end < rollingEnd ? effectiveRange.end : rollingEnd;
  const evidence = [createEvidence({
    targetId: 'barnard-fitness',
    sourceId: 'barnardFitness',
    priority: 3,
    effectiveRange: { start: generatedDate, end: liveEnd },
    weeklyIntervals,
    reason: approvedStaleHeading ? BARNARD_STALE_HEADING_NOTE : null,
    availabilityType: 'facility-hours',
    accessRestrictions,
  })];
  if (liveEnd < rollingEnd) {
    evidence.push(unavailableEvidence({
      targetId: 'barnard-fitness',
      sourceId: 'barnardFitness',
      priority: 3,
      effectiveRange: { start: addDays(liveEnd, 1), end: rollingEnd },
      unavailableStatus: 'Hours need verification',
      availabilityType: null,
      accessRestrictions,
    }));
...
function seasonalLabel(value) {
  return SEASONAL_HEADING_EXPRESSION.test(boundedText(value) || '');
}

function seasonalHeadingYear(value) {
  const match = boundedText(value)?.match(SEASONAL_HEADING_EXPRESSION);
  return match ? Number(match[1]) : null;
}

function barnardSeasonHeadingState(value, generatedDate) {
  const match = boundedText(value)?.match(/\b(spring|summer|fall|winter)\s+(20\d{2})\b/i);
  if (!match || match[2] !== generatedDate.slice(0, 4)) return null;
  const season = match[1].toLowerCase();
  const month = Number(generatedDate.slice(5, 7));
  const accepted = month <= 2
    ? ['winter', 'spring']
    : month <= 4
      ? ['winter', 'spring']
      : month <= 8
        ...
        : month <= 11
          ? ['summer', 'fall']
          : ['fall', 'winter'];
  return accepted.includes(season);
}
...
function targetIdForColumbiaScheduleHeading(heading) {
  ...
  if (/^(?:summer|fall|spring|winter)\s+(?:(?:semester|session)\s+)?building hours$/i.test(boundedHeading || '')) {
    return 'dodge';
  }
...
/**
 * Fall 2026 wording: the seasonal paragraph dropped the "effective <Month D, YYYY> through
 * <Month D, YYYY>" form that parseDateRange needs, for
 *
 *   "Dodge Fitness Center will operate on an academic semester schedule from
 *    Saturday, September 5 through Wednesday, December 23."
 *
 * — no year, and every date carries its weekday. Without a bounded range the parser
 * refuses to publish seasonal times (by design), which is what left Uris Pool reading
 * "Hours need verification" for the whole Fall term.
 *
 * The year comes from the seasonal heading ("Fall Semester 2026"), an end before the
 * start rolls into the next year (a winter term), and BOTH weekday names must agree with
 * the dates they imply — so a misparse fails closed instead of quietly widening the range.
 * The sentence lives outside the schedule container, so this scans the document like
 * seasonalTransitionDateRange does, and refuses anything but a single unambiguous match.
 */
function seasonalSemesterDateRange($, seasonalHeading) {
  const year = seasonalHeadingYear(seasonalHeading.text());
  if (!year) return null;
  const paragraphs = $('p').map((_, element) => boundedText($(element).text(), 500)).get().filter(Boolean);
  const matches = paragraphs
    .map(text => text.match(SEMESTER_SCHEDULE_EXPRESSION))
    .filter(Boolean);
  if (matches.length !== 1) return null;

  const [, startWeekday, startMonth, startDay, endWeekday, endMonth, endDay] = matches[0];
  const start = numericDate(year, monthNumber(startMonth), Number(startDay));
  if (!start) return null;
  const end = numericDate(year, monthNumber(endMonth), Number(endDay));
  const bounded = end && end >= start
    ? end
    : numericDate(year + 1, monthNumber(endMonth), Number(endDay));
  if (!bounded || bounded < start) return null;
  if (weekdayForDate(start) !== startWeekday.toLowerCase()) return null;
  if (weekdayForDate(bounded) !== endWeekday.toLowerCase()) return null;
...
function isOfficialActivityCalendar(value, definition) {
  try {
    const url = new URL(value);
    const titles = url.searchParams.getAll('title');
    const validTitle = definition.title === null
      ? titles.length === 0
      : titles.length === 1 && titles[0] === definition.title;
    return validTitle
      && url.protocol === 'https:'
...
function calendarEvents(text) {
  ...
  for (const match of text.matchAll(pattern)) {
    const date = parseSingleDate(match[5]);
    const interval = parseTimeRange(`${match[1]} - ${match[2]}`);
    const title = boundedText(match[3]);
    const calendarName = boundedText(match[4]);
    if (date && interval && title && calendarName) events.push({ date, interval, title, calendarName });
  }
  return events;
}
...
function parseTimeRange(value) {
  ...
  // A closing time written as "12 AM" is the end of that day, which the schema spells
  // 24:00 (lib/recreation-hours-schema.js CLOSE_TIME). Fall 2026 is the first Columbia
  // schedule to close at midnight — Dodge does so Sunday through Thursday — and without
  // this the row parsed as 06:00–00:00, failed the start < end check, and was dropped.
  if (interval[1] === '00:00' && interval[0] !== '00:00') interval[1] = '24:00';
  return interval[0] < interval[1] ? interval : null;
}

/* Barnard writes split shifts as "7-9:50am / 2:30-9pm" and shares one
   meridiem across a range. Returns every interval on the row, in order,
   or null if any piece fails to parse. */
function parseBarnardTimeRanges(value) {
  const text = boundedText(value);
  if (!text) return null;
  const pieces = text.split(/\s*(?:\/|,|&|\band\b)\s*/i).filter(Boolean);
  if (!pieces.length) return null;
  const intervals = [];
  for (const piece of pieces) {
    const interval = parseTimeRange(piece) ?? parseSharedMeridiemRange(piece);
    if (!interval) return null;
    intervals.push(interval);
  }
  for (let i = 1; i < intervals.length; i++) {
    if (intervals[i][0] < intervals[i - 1][1]) return null; // overlapping or out of order
  }
  return intervals;
}

/* "7-9:50am" → the start borrows the end's meridiem; "11-1pm" → flips it
   when borrowing would put the start after the end. */
function parseSharedMeridiemRange(value) {
  const text = boundedText(value);
  const match = text?.match(/^(1[0-2]|[1-9])(?::([0-5]\d))?\s*[-–]\s*(1[0-2]|[1-9])(?::([0-5]\d))?\s*(a\.?m\.?|p\.?m\.?)$/i);
  if (!match) return null;
  const end = to24Hour(match[3], match[4], match[5]);
  const endIsPm = /^p/i.test(match[5]);
  const sameMeridiem = to24Hour(match[1], match[2], match[5]);
  if (sameMeridiem < end) return [sameMeridiem, end];
  const flipped = to24Hour(match[1], match[2], endIsPm ? 'am' : 'pm');
  return flipped < end ? [flipped, end] : null;
}
...
function parseUnambiguousEffectiveRange(value) {
  const text = boundedText(value, 500);
  if (!text) return null;
  const pattern = /effective\s+([A-Za-z]+\s+\d{1,2},\s+\d{4})\s+(?:through|to|-)\s+([A-Za-z]+\s+\d{1,2},\s+\d{4})/gi;
  const matches = [...text.matchAll(pattern)];
  if (matches.length !== 1) return null;
  const match = matches[0];
  const remaining = `${text.slice(0, match.index)}${text.slice(match.index + match[0].length)}`;
  if (/\beffective\b/i.test(remaining)) return null;
  const start = parseSingleDate(match[1]);
  const end = parseSingleDate(match[2]);
  return start && end && start <= end ? { start, end } : null;
}
