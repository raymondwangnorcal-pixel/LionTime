...
function validateLaborPayload(value, path, errors) {
  ...
  const venueIds = new Set(Object.values(DINING_LOCATION_CONTRACT).map(contract => contract.id));
  value.days.forEach((day, dayIndex) => {
    const dayPath = `${path}.days[${dayIndex}]`;
...
export function validateDiningAttemptBatch(value) {
  ...
  if (!Array.isArray(value.attempts) || value.attempts.length !== DINING_SOURCE_IDS.length) {
    errors.push('attempts must contain four sources');
  } else {
    value.attempts.forEach((attempt, index) => {
      ...
      const sourceId = DINING_SOURCE_IDS[index];
      if (attempt.sourceId !== sourceId || attempt.sourceUrl !== DINING_SOURCE_CONTRACT[sourceId]) {
...
export function validateDiningSourceState(value) {
  ...
  if (value.kind !== 'dining-source-state' || value.schemaVersion !== 1
    || !validTimestamp(value.generated)) errors.push('source state metadata is invalid');
  if (!Array.isArray(value.sources) || value.sources.length !== DINING_SOURCE_IDS.length) {
    errors.push('source state must contain four sources');
  } else {
    value.sources.forEach((source, index) => {
      ...
      const sourceId = DINING_SOURCE_IDS[index];
      if (source.sourceId !== sourceId || source.sourceUrl !== DINING_SOURCE_CONTRACT[sourceId]) {
        errors.push(`${path} source identity is invalid`);
      }
