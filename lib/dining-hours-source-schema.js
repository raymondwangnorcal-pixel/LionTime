...
export function validateDiningAttemptBatch(value) {
  ...
  if (!Array.isArray(value.attempts) || value.attempts.length !== DINING_SOURCE_IDS.length) {
    errors.push('attempts must contain five sources');
  } else {
...
export function validateDiningSourceState(value) {
  ...
  const sourceIds = value.schemaVersion === 1 ? LEGACY_DINING_SOURCE_IDS : DINING_SOURCE_IDS;
  const sourceContract = value.schemaVersion === 1 ? LEGACY_DINING_SOURCE_CONTRACT : DINING_SOURCE_CONTRACT;
  if (value.kind !== 'dining-source-state' || ![1, 2].includes(value.schemaVersion)
