    ...
    async handle(request) {
      try {
        ...
        if (request.method === 'POST') {
          ...
          /* hallId: null → unvote; otherwise must be a valid hall */
          const isUnvote = hallId === null;
          if (!isUnvote && (!hallId || !VALID_HALL_IDS.has(hallId))) {
            return {
              status: 400,
              headers: { ...corsHeaders(), 'Cache-Control': 'no-store' },
          ...
          if (isUnvote) {
            await store.removeVote(date, fp);
          } else {
