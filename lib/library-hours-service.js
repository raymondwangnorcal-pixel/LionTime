...
export function createLibraryHoursService({ store, updateSecret, logger = console }) {
  return {
    async handle(request) {
      try {
        if (request.method === 'GET') {
          ...
          return snapshot
            ...
            : {
                ...
                body: { error: 'Library hours are not initialized' },
              };
        }
        ...
        logger.error('Library hours storage operation failed', { name: error?.name });
        return {
          status: 500,
          headers: { 'Cache-Control': 'no-store' },
