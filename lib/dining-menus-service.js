...
import { validateDiningMenusSnapshot } from './dining-menus-schema.js';
...
export function createDiningMenusService({ store, updateSecret, logger = console }) {
  return {
    async handle(request) {
      try {
        if (request.method === 'GET') {
          ...
          return snapshot
            ...
            : {
                ...
                body: { error: 'Dining menus are not initialized' },
              };
        }
        ...
        const validation = validateDiningMenusSnapshot(request.body);
        if (!validation.ok) {
          return {
            status: 422,
            headers: { 'Cache-Control': 'no-store' },
        ...
        logger.error('Dining menus storage operation failed', { name: error?.name });
