...
/**
 * dineoncampus publishes meal periods, not opening hours, and ends one a minute before the
 * next begins: Diana Center Cafe came out as 11:30-14:59, 15:00-16:59, 17:00-00:00 — four
 * ranges for a cafe that never shuts. Merging only what strictly overlapped left those
 * boundaries intact, so a continuous day read as a day full of gaps.
 *
 * Intervals that touch, or that sit within SLOT_BOUNDARY_MINUTES of each other, are now one
 * interval. The tolerance is one minute on purpose: it closes the slot-boundary artefact and
 * nothing else. A real break — Diana's 11:00-11:30 between breakfast and lunch — is thirty
 * minutes wide and still shows.
 */
const SLOT_BOUNDARY_MINUTES = 1;

export function normalizeIntervals(values) {
  const parsed = values.map(parseBarnardTimeRange)
  ...
  for (const interval of parsed) {
    ...
    if (merged.length && start <= merged[merged.length - 1].end + SLOT_BOUNDARY_MINUTES) {
      const prev = merged[merged.length - 1];
      if (end > prev.end) { prev.end = end; prev.close = interval[1]; }
    } else {
      merged.push({ open: interval[0], close: interval[1], end });
    }
  }
  return merged.map(({ open, close }) => [open, close]);
}
