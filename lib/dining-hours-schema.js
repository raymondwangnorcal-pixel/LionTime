...
export const LEGACY_DINING_SOURCE_CONTRACT = Object.freeze({
...
export const DINING_SOURCE_CONTRACT_V3 = Object.freeze({
...
export const LEGACY_DINING_LOCATION_CONTRACT = Object.freeze({
...
export const DINING_LOCATION_CONTRACT_V3 = Object.freeze({
  ...LEGACY_DINING_LOCATION_CONTRACT,
  'cafe-east': Object.freeze({ id: 'cafe-east', category: 'cafe' }),
});

export const DINING_LOCATION_CONTRACT = Object.freeze({
  ...DINING_LOCATION_CONTRACT_V3,
  hewitt: Object.freeze({ id: 'hewitt', category: 'dining', sourceId: 'barnard-hours' }),
  'diana-center-cafe': Object.freeze({
    id: 'diana-center-cafe', category: 'dining', sourceId: 'barnard-hours',
  }),
  'barnard-bubble-tea-sushi': Object.freeze({
    id: 'barnard-bubble-tea-sushi', category: 'dining', sourceId: 'barnard-hours',
  }),
  'lizs-place': Object.freeze({ id: 'lizs-place', category: 'cafe', sourceId: 'barnard-hours' }),
});
...
function validateVersionedSources(value, errors, sourceContract, requireGeneratedTimestamp) {
  const sourceIds = Object.keys(sourceContract);
  if (!Array.isArray(value.sources) || value.sources.length !== sourceIds.length) {
    errors.push(`sources must contain ${sourceIds.length} official sources`);
    return;
  }
  const seen = new Set();
  for (const [index, source] of value.sources.entries()) {
    ...
    if (!(source.id in sourceContract) || seen.has(source.id)) {
      errors.push(`${path}.id must be a unique official source`);
    }
    seen.add(source.id);
    if (source.url !== sourceContract[source.id]) errors.push(`${path}.url does not match source ID`);
    if (!validTimestamp(source.fetchedAt)) errors.push(`${path}.fetchedAt must be timezone-aware`);
    if (requireGeneratedTimestamp && source.fetchedAt !== value.generated) {
      errors.push(`${path}.fetchedAt must equal generated`);
    }
  }
...
export function validateDiningHoursSnapshot(value) {
  ...
  const isVersionThree = value.schemaVersion === 3;
  const isVersionFour = value.schemaVersion === 4;
  const hasProvenance = isVersionTwo || isVersionThree || isVersionFour;
  if (value.schemaVersion !== 1 && !hasProvenance) errors.push('schemaVersion must be 1, 2, 3, or 4');
  if (hasProvenance && !exactKeys(value, [
    'schemaVersion', 'generated', 'source', 'windowStart', 'windowEnd', 'sources', 'locations', 'specialServices',
  ])) errors.push(`version ${value.schemaVersion} snapshot contains unexpected fields`);
  ...
  const locationContract = isVersionFour ? DINING_LOCATION_CONTRACT
    : isVersionThree ? DINING_LOCATION_CONTRACT_V3
      : LEGACY_DINING_LOCATION_CONTRACT;
  const sourceContract = isVersionFour ? DINING_SOURCE_CONTRACT
    : isVersionThree ? DINING_SOURCE_CONTRACT_V3
      : LEGACY_DINING_SOURCE_CONTRACT;
  const contractById = new Map(Object.entries(locationContract)
    .map(([sourceId, contract]) => [contract.id, { ...contract, sourceId: contract.sourceId || sourceId }]));
  ...
  for (const [index, location] of value.locations.entries()) {
    ...
    if (hasProvenance && !exactKeys(location, ['id', 'sourceId', 'category', 'name', 'days'])) {
      errors.push(`${path} contains unexpected fields`);
    }
    ...
    if (typeof location.sourceId !== 'string') errors.push(`${path}.sourceId must be a string`);
    ...
    for (let dayIndex = 0; dayIndex < 14; dayIndex += 1) {
      ...
      if (hasProvenance) {
        ...
        if (contract?.sourceId === 'barnard-hours'
          && !['barnard-hours', 'unpublished'].includes(day.sourceId)) {
          errors.push(`${dayPath}.sourceId cannot override Barnard evidence`);
        }
  ...
  for (const { id } of Object.values(locationContract)) {
    if (!seenIds.has(id)) errors.push(`missing required location: ${id}`);
  }
  if (hasProvenance) {
    validateVersionedSources(value, errors, sourceContract, !isVersionFour);
