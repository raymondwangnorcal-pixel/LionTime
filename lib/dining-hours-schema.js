...
function validateVersionedSources(value, errors, sourceContract) {
...
export function validateDiningHoursSnapshot(value) {
  ...
  const hasProvenance = isVersionTwo || isVersionThree;
  if (value.schemaVersion !== 1 && !hasProvenance) errors.push('schemaVersion must be 1, 2, or 3');
  ...
  const locationContract = isVersionThree ? DINING_LOCATION_CONTRACT : LEGACY_DINING_LOCATION_CONTRACT;
  const sourceContract = isVersionThree ? DINING_SOURCE_CONTRACT : LEGACY_DINING_SOURCE_CONTRACT;
  ...
  for (const sourceId of Object.keys(locationContract)) {
    if (!seenSources.has(sourceId)) errors.push(`missing required source: ${sourceId}`);
  }
  if (hasProvenance) {
    validateVersionedSources(value, errors, sourceContract);
