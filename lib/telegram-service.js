...
export function createTelegramService({
  ...
  actions = ACTIONS,
  tickets = null,                     // optional generic card-tap recorder (lib/telegram-tickets.js)
} = {}) {
  ...
  async function handleCallback(callback, { from, chat }) {
    if (tickets) {
      const handled = await tickets.handleCallback(callback, { from, chat });
   ...
   *   markups?: Array<{ chatId: number|string, messageId: number, replyMarkup: object }>,
   * }>}
   */
  async function handle({ secretHeader, update }) {
    ...
    // A plain (non-command) message may be the text an edit tap is waiting for.
    if (tickets && update.message && text && !text.startsWith('/')) {
      const handled = await tickets.handleText(text, { from, chat });
      if (handled) return handled;
    }
