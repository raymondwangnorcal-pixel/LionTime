import {
  BARNARD_MANUAL_SOURCE_ID,
  DODGE_SPACES,
  RECREATION_FACILITIES,
  RECREATION_MANUAL_SOURCE_IDS,
  RECREATION_SOURCE_URLS,
} from './recreation-hours-catalog.js';
import { matchesApprovedBarnardManualDay } from './recreation-hours-manual-overrides.js';
...
const TRUSTED_SOURCE_IDS = new Set([
  ...Object.keys(RECREATION_SOURCE_URLS),
  ...RECREATION_MANUAL_SOURCE_IDS,
]);
const COLUMBIA_SOURCE_IDS = new Set(['columbiaHours', 'columbiaModifications']);
const BARNARD_SOURCE_IDS = new Set(['barnardFitness', ...RECREATION_MANUAL_SOURCE_IDS]);
...
function allowedSourcesFor(targetId) {
  return targetId === 'barnard-fitness' ? BARNARD_SOURCE_IDS : COLUMBIA_SOURCE_IDS;
}
...
function validateSourceRefs(value, { path, targetId, errors }) {
  ...
  for (const [index, sourceRef] of value.entries()) {
    if (!TRUSTED_SOURCE_IDS.has(sourceRef)) {
...
function validateEvidenceRefs(value, { path, targetId, isDodgeChild, sourceRefs, errors }) {
  ...
  for (const [index, evidenceRef] of value.entries()) {
    ...
    if (!TRUSTED_SOURCE_IDS.has(sourceId)
...
function usesBarnardManualSource(value) {
  return Array.isArray(value?.sourceRefs) && value.sourceRefs.includes(BARNARD_MANUAL_SOURCE_ID)
    || Array.isArray(value?.evidenceRefs)
      && value.evidenceRefs.some(ref => typeof ref === 'string' && ref.startsWith(`${BARNARD_MANUAL_SOURCE_ID}:`));
}
...
export function validateRecreationHoursSnapshot(value) {
  ...
  validateExactKeys(value, new Set(['generated', 'facilities', 'accessDenied']), 'snapshot', errors);
  ...
  if ('accessDenied' in value) {
    if (!Array.isArray(value.accessDenied)) {
      errors.push('accessDenied must be an array');
    } else {
      for (const [index, entry] of value.accessDenied.entries()) {
        const entryPath = `accessDenied[${index}]`;
        if (!isRecord(entry)) {
          errors.push(`${entryPath} must be an object`);
          continue;
        }
        validateExactKeys(entry, new Set(['id', 'name']), entryPath, errors);
        if (typeof entry.id !== 'string' || !Object.hasOwn(RECREATION_FACILITIES, entry.id)) {
          errors.push(`${entryPath}.id must reference a known facility`);
        }
        if (typeof entry.name !== 'string' || !validText(entry.name)) {
          errors.push(`${entryPath}.name must be bounded plain text`);
        }
      }
    }
  }
