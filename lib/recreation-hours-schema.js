...
function availabilityAllowedFor(targetId, availabilityType) {
  ...
  if (DODGE_SPACES[targetId]) return ['open-recreation', 'reservation-required'].includes(availabilityType);
...
function validateDay(day, { path, expectedDate, errors, targetId, isSpace = false }) {
  ...
  if (day.availabilityType === 'reservation-required' && day.status !== 'Reservation required') {
    errors.push(`${path}.reservation-required availability must use Reservation required status`);
  }
...
function isVerifiedDodgeClosure(day) {
  return day.validIntervals
    ...
    && day.status !== 'Separate hours not published'
...
function validateChildAgainstDodge({ child, parent, targetId, index, errors, isSpace }) {
  if (!parent.validIntervals || !child.validIntervals) return;
  ...
  if (child.intervals.some(interval => !intervalIsWithin(interval, parent.intervals))) {
    errors.push(`${targetId} day ${index} intervals must be within Dodge hours`);
  }
  if (child.conflict === true && child.status !== 'Hours need verification') {
    errors.push(`${targetId} day ${index} non-inherited conflict must use Hours need verification`);
  }
}
