...
| Barnard Physical Well-Being / Fitness Center | `https://barnard.edu/lefrak-center/physical-well-being` | Exact-bounded Barnard Fitness Center hours and separate access restrictions. Beginning September 8, 2026, a complete seven-day seasonal schedule with recognized Barnard access copy is treated as the live rolling fourteen-day schedule displayed by Barnard; partial or ambiguous rows fail the source atomically. |

The separate trusted manual-source list contains `barnardManualOverride` only. It records the user-confirmed one-time Barnard Fitness schedule for August 24 through September 7, 2026: closed August 24; 9 AM–2 PM August 25; 9 AM–7 PM August 26–27; and closed August 28–September 7. Its explicit date ranges make it inert after September 7. Beginning with the first scrape on September 8, the normal `barnardFitness` source resumes and publishes the weekly schedule displayed on Barnard's page as a rolling fourteen-day schedule. This manual provenance ID is accepted only for Barnard Fitness and must not be represented as an official-page scrape.
...
- Barnard labelled its table `Summer 2026` without exact effective endpoints. Through September 7, its times remain subordinate to the explicit manual schedule. Beginning September 8, the displayed weekly times publish as rolling live evidence; while the heading still says exactly `Summer 2026`, LionHour adds the small note `Barnard's seasonal heading may be outdated.` Once Barnard updates to a season relevant to the scrape date, the note disappears without changing the live-source path. Explicit effective dates must be unambiguous; if they end inside the fourteen-day window, later days remain verification-only while retaining Barnard access and provenance.
