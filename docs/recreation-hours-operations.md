...
Dodge data carries exactly five nested spaces: Blue Gym, Levien Gymnasium, Functional Fitness Studio, Aerobics Room 4, and Squash Courts. The LionHour interface currently displays only the four calendar-backed spaces and filters the booking-only Squash Courts row from view. No Dodge space becomes an additional top-level card. Each snapshot must cover fourteen consecutive Eastern dates for every top-level facility and nested space.
