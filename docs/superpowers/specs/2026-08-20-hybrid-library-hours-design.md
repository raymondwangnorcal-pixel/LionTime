...
- Scrape the seven libraries displayed by LionHour from `https://hours.library.columbia.edu/locations/<slug>`.
- For Milstein Library only, overlay exact holiday closures from `https://library.barnard.edu/visit/hours` on the Columbia Barnard calendar.
...
Changing the site's visual design and making non-library venue hours dynamic are outside this change. DEC-0033 extends the original six-library scope with Milstein Library.

## Architecture

The browser continues to receive static `index.html`. It renders immediately with the existing built-in hours, requests `GET /api/library-hours`, validates the response, overlays the seven library schedules, and renders again. Failure of the API request never blocks the rest of the page.
...
Milstein's display identity is independent of its official source title. Its primary daily schedule is the ISO-dated Columbia `barnard` calendar. The Barnard Library hours page is a secondary source used only for the table headed `Upcoming Holidays and Library Closures`. Each closure must be bounded by a nearby schedule heading containing the applicable year and must publish a weekday that matches the computed calendar date. A validated exact closure replaces the primary interval for that date. Missing, ambiguous, stale-year, or malformed holiday evidence makes the Milstein entry unpublishable; the atomic publisher retains the prior complete snapshot.

```mermaid
flowchart LR
  C["Columbia library-hours pages"] --> S["GitHub Actions scraper"]
  H["Barnard Library holiday closures"] --> S
...
Validation requires schema version `1`, an ISO timestamp, exactly the seven displayed scraper IDs, canonical `HH:MM` times, and source URLs under `hours.library.columbia.edu`. A normal entry requires a schedule covering the generated Eastern date. Only Lehman may instead use `useEmbeddedFallback: true`, `fallbackReason: "unapproved-overnight-hours"`, and an empty schedule array. The `barnard` record must use the exact primary URL `https://hours.library.columbia.edu/locations/barnard` and the exact `holidayUrl` `https://library.barnard.edu/visit/hours`; no other record may carry `holidayUrl`.

Published snapshots contain exactly the seven displayed libraries. `temporarilyClosed` must be Boolean; when true, the active schedule must contain only closed days. An embedded-fallback entry cannot also be temporarily closed or carry dynamic schedules.

## Reliability and Safety

- A scrape is publishable when every normal entry parses successfully and covers the generated date; the single approved Lehman anomaly may explicitly retain its embedded schedule while the other six update.
- Milstein requires both official sources to parse. Barnard's source can only close an exact date; it cannot invent an opening interval or replace Columbia's ordinary schedule.
...
- The Libraries section shows the snapshot's generated time. Data older than eight hours, or embedded fallback data used after an API failure, is visibly marked stale and links to Columbia's source hours. A fresh partial snapshot reports that six of seven libraries are live and that Lehman is using its embedded fallback.
...
- Python unit tests cover time parsing, explicit closed days, parse errors, stable identity mapping, calendar extraction, exact year-bounded Barnard holiday closures, holiday precedence, schedule selection, complete-snapshot validation, suspicious overnight rejection, and failed-library behavior using captured official HTML fixtures.
